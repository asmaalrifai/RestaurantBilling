...
void generateBillHeader(char customer[MAX_STRING], char phone[MAX_STRING], char date[MAX_STRING]) {
    printf("\n\n");
...
// Function to save an invoice to a file
...
void generateInvoice() {
    ...
    generateBillHeader(currentOrder.customerInfo.name, currentOrder.customerInfo.phone, currentOrder.date);
...
void showAllInvoices() {
    ...
    while (fread(&savedOrder, sizeof(struct Order), 1, fp)) {
        float tot = 0;
        generateBillHeader(savedOrder.customerInfo.name, savedOrder.customerInfo.phone, savedOrder.date);
...
void searchInvoice() {
    ...
    while (fread(&savedOrder, sizeof(struct Order), 1, fp)) {
        float tot = 0;
        if (!strcmp(savedOrder.customerInfo.name, name)) {
            generateBillHeader(savedOrder.customerInfo.name, savedOrder.customerInfo.phone, savedOrder.date);
