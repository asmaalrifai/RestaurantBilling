...
struct orders
{
    customer *customer; // pointer to struct customer
...
// Function to generate bill header
void generateBillHeader(customer *customer, char date[MAX_STRING])
{
    printf("\n\n");
...
int main(void)
{
    ...
    while (contFlag == 'y')
    {
        ...
        case 1:
            ...
            ord.customer = malloc(sizeof(customer));
            ...
            if (saveBill == 'y')
            {
                ...
                if (fp != NULL)
                {
                    fprintf(fp, "%s %s %s %d ", ord.customer->name, ord.customer->phone, ord.date, ord.numOfItems);
                    for (size_t i = 0; i < ord.numOfItems; i++)
                    {
                        fprintf(fp, "%s %d %.2f ", ord.itm[i].item[0], ord.itm[i].qty, ord.itm[i].price);
                    }
                    fprintf(fp, "\n");
        ...
        case 2:
            ...
            if (fp != NULL)
            {
                ...
                printf("\nfyu %d",fscanf(fp, "%s", name));
                order.customer=malloc(sizeof(customer));
                strcpy(order.customer->name,name);
                /*while (fscanf(fp, "%s %s %s %d", order.customer->name, order.customer->phone, order.date, &order.numOfItems) == 4)
                {
                    printf("ffj");
                    // Print bill details
            ...
            if (fp != NULL)
            {
                ...
                while (fscanf(fp, "%s %s %s %d", order.customer->name, order.customer->phone, order.date, &order.numOfItems) == 4)
                {
                    ...
                    if (_stricmp(order.customer->name, name) == 0)
                    {
                        for (size_t i = 0; i < order.numOfItems; i++)
                        {
                            fscanf(fp, "%s %d %f", order.itm[i].item[0], &order.itm[i].qty, &order.itm[i].price);
                        }
                        float tot = order.calculateTotal(order.itm, order.numOfItems);
