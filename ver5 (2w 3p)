...
{
    struct orders ord;
    while (fscanf(fp, "%s %s %s %d", ord.customer->name, ord.customer->phone, ord.date, &ord.numOfItems) != EOF)
    {
        ord.itm = malloc(ord.numOfItems * sizeof(struct items));

        for (int i = 0; i < ord.numOfItems; i++)
        {
            ord.itm[i].item = malloc(sizeof(char *));
            ord.itm[i].item[0] = malloc(MAX_STRING * sizeof(char));
            fscanf(fp, "%s %d %f", ord.itm[i].item[0], &ord.itm[i].qty, &ord.itm[i].price);
        }
...
void searchInvoice(FILE *fp)
{
    ...
    while (fscanf(fp, "%s %s %s %d", ord.customer->name, ord.customer->phone, ord.date, &ord.numOfItems) != EOF)
    {
        ...
        if (strcmp(ord.customer->name, searchName) == 0)
        {
            printBillDetails(&ord);
            found = 1;
            break;
...
int main(void)
{
    ...
    while (contFlag == 'y')
    {
        ...
        {
        case 1:
            fp = fopen("RestaurantBill.txt", "a+t");
            if (fp != NULL)
            {
                generateInvoice(fp);
                fclose(fp);
        ...
        case 2:
    fp = fopen("RestaurantBill.txt", "rt");
    if (fp != NULL)
    {
        showAllInvoices(fp);
        fclose(fp);
